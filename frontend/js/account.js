...
async function openJournal(journalId) {
    ...
    try {
        ...
        window.currentGradingScheme   = journal.grading_scheme || "1_5";
        ...
        // Переходим на нужную вкладку в зависимости от типа журнала
        const targetPage = journal.journal_type === "practice" ? "grades" : "attendance";
        document.querySelectorAll(".page").forEach(p => p.classList.remove("active"));
        document.getElementById(targetPage)?.classList.add("active");
        document.querySelectorAll(".btn[data-page]").forEach(b => b.classList.remove("active"));
        document.querySelector(`.btn[data-page='${targetPage}']`)?.classList.add("active");
