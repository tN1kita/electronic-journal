...
function buildJournalTables(journal, lessons, students) {
    ...
    // Лекция: показываем посещаемость, скрываем оценки
    // Практика: показываем оценки, скрываем посещаемость
    if (isLecture) {
        buildTable("attendanceTable", journal, lessons, students, "attendance");
        enableAttendanceTab();
        disableGradesTab();
    } else {
        disableAttendanceTab();
        ...
        try {
            if (val === "") {
                ...
                if (mode === "grades") {
                    ...
                    if (scheme === "0_100" && (grade < 0 || grade > 100)) return;
                    if (scheme === "1_5"   && (grade < 1 || grade > 5))   return;
...
function getScheme() {
    return window.currentGradingScheme
        || window.currentJournalData?.grading_scheme
        || "1_5";
}
...
function applyGradeStyle(cell) {
    cell.classList.remove(
        ...
        "grade-0_100-bad","grade-0_100-low","grade-0_100-mid","grade-0_100-good"
    );
    ...
    if (getScheme() === "0_100") {
        const n = parseInt(v);
        if (isNaN(n)) return;
        if (n < 50)      cell.classList.add("grade-0_100-bad");
        else if (n < 70) cell.classList.add("grade-0_100-low");
        else if (n < 90) cell.classList.add("grade-0_100-mid");
        else             cell.classList.add("grade-0_100-good");
    } else {
        if (["1","2","3","4","5"].includes(v)) cell.classList.add(`grade-${v}`);
    }
}

function enableAttendanceTab() {
    const btn = document.querySelector(".btn[data-page='attendance']");
    btn?.classList.remove("disabled");
    btn?.removeAttribute("disabled");
    const page = document.getElementById("attendance");
    if (!page) return;
    page.querySelector(".attendance-unavailable")?.remove();
...
function disableAttendanceTab() {
    ...
    let notice = page.querySelector(".attendance-unavailable");
    if (!notice) {
        notice = document.createElement("div");
        notice.className = "attendance-unavailable grades-unavailable";
        notice.innerHTML = `
            <div class="grades-unavailable-inner">
                ...
                <h3>Посещаемость недоступна</h3>
                <p>Журнал создан как <strong>Практический</strong>.<br>Вкладка посещаемости доступна только для лекционных журналов.</p>
            </div>`;
    ...
    if (!notice) {
        ...
        notice.innerHTML = `
            <div class="grades-unavailable-inner">
                ...
                <h3>Посещаемость недоступна</h3>
                <p>Журнал создан как <strong>Практика</strong>.<br>Вкладка посещаемости доступна только для лекционных журналов.</p>
            </div>`;
        page.appendChild(notice);
    }
    const wrapper = page.querySelector(".table-wrapper");
